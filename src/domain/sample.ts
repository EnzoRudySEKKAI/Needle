...
export const SAMPLE_MAP: OntologyDocument = {
  ...
  nodes: [
    ...
    { id: 'intake', code: 'IN', name: 'Evidence intake', groupId: 'sense', floorId: 'sense-floor', size: 'm', position: { gx: 0, gy: 2 }, faceTexture: 'plain', properties: [{ id: 'p-in-1', key: 'required', value: 'time, place, source' }], whatItDoes: 'Turns incoming reports into comparable evidence without erasing where they came from.', howItsBuilt: 'Normalization adds a common envelope and preserves the original payload beside it.' },
    { id: 'correlation', code: 'CR', name: 'Correlation', groupId: 'sense', floorId: 'sense-floor', size: 'l', position: { gx: 4, gy: 2 }, faceTexture: 'hatched', properties: [{ id: 'p-cr-1', key: 'window', value: '72 hours' }, { id: 'p-cr-2', key: 'threshold', value: '3 sources' }], whatItDoes: 'Finds observations that describe the same change across time, place and source.', howItsBuilt: 'A claim only advances when independent evidence agrees; volume alone is not treated as confidence.' },
    { id: 'review', code: 'RV', name: 'Expert review', groupId: 'decision', floorId: 'response-floor', size: 'm', position: { gx: 1, gy: 1 }, faceTexture: 'plain', properties: [{ id: 'p-rv-1', key: 'outcomes', value: 'watch, alert, reject' }], whatItDoes: 'Decides whether a pattern deserves action and records the reason for that decision.', howItsBuilt: 'The decision is explicit rather than inferred from a score, keeping accountability with the reviewer.' },
    { id: 'dispatch', code: 'DP', name: 'Response dispatch', groupId: 'decision', floorId: 'response-floor', size: 'm', position: { gx: 5, gy: 1 }, faceTexture: 'plain', properties: [{ id: 'p-dp-1', key: 'priority', value: 'confidence × urgency' }], whatItDoes: 'Routes a verified alert to the people who can act in the affected place.', howItsBuilt: 'Recipients are selected from geography and responsibility instead of a single broadcast list.' },
    { id: 'steward', code: 'ST', name: 'Local steward', groupId: 'outside', floorId: 'response-floor', size: 's', position: { gx: 2, gy: 7 }, faceTexture: 'plain', properties: [{ id: 'p-st-1', key: 'response', value: 'inspect and report' }], whatItDoes: 'Checks the place, acts when needed and reports what changed after the intervention.', howItsBuilt: 'The response closes the evidence loop instead of ending at notification delivery.' },
    { id: 'habitat', code: 'HB', name: 'Habitat', groupId: 'outside', floorId: 'response-floor', size: 'xl', position: { gx: 7, gy: 7 }, faceTexture: 'hatched', properties: [{ id: 'p-hb-1', key: 'state', value: 'observed over time' }], whatItDoes: 'The living place the whole system is trying to understand and protect.', howItsBuilt: 'It is modelled as a first-class concept so every alert and response remains tied to a real place.' },
  ],
  relations: [
