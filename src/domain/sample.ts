...
export const SAMPLE_MAP: OntologyDocument = {
  ...
  nodes: [
    { id: 'observer', code: 'OB', name: 'Observer', groupId: 'field', size: 's', position: { gx: 0, gy: 0 }, faceTexture: 'plain', properties: [{ id: 'p-ob-1', key: 'channel', value: 'mobile capture' }], whatItDoes: 'Records a change in the field while its location and context are still known.', howItsBuilt: 'Observations stay attributable to a person so later decisions can return to the original witness.' },
    { id: 'sensor', code: 'SN', name: 'Sensor mesh', groupId: 'field', size: 'l', position: { gx: 4, gy: 0 }, faceTexture: 'hatched', properties: [{ id: 'p-sn-1', key: 'cadence', value: '15 minutes' }, { id: 'p-sn-2', key: 'coverage', value: '12 zones' }], whatItDoes: 'Supplies continuous measurements where a person cannot remain present.', howItsBuilt: 'Readings are kept as a separate evidence stream because machine confidence and human confidence mean different things.' },
    { id: 'intake', code: 'IN', name: 'Evidence intake', groupId: 'sense', size: 'm', position: { gx: 0, gy: 7 }, faceTexture: 'plain', properties: [{ id: 'p-in-1', key: 'required', value: 'time, place, source' }], whatItDoes: 'Turns incoming reports into comparable evidence without erasing where they came from.', howItsBuilt: 'Normalization adds a common envelope and preserves the original payload beside it.' },
    { id: 'correlation', code: 'CR', name: 'Correlation', groupId: 'sense', size: 'l', position: { gx: 4, gy: 7 }, faceTexture: 'hatched', properties: [{ id: 'p-cr-1', key: 'window', value: '72 hours' }, { id: 'p-cr-2', key: 'threshold', value: '3 sources' }], whatItDoes: 'Finds observations that describe the same change across time, place and source.', howItsBuilt: 'A claim only advances when independent evidence agrees; volume alone is not treated as confidence.' },
    { id: 'review', code: 'RV', name: 'Expert review', groupId: 'decision', size: 'm', position: { gx: 10, gy: 4 }, faceTexture: 'plain', properties: [{ id: 'p-rv-1', key: 'outcomes', value: 'watch, alert, reject' }], whatItDoes: 'Decides whether a pattern deserves action and records the reason for that decision.', howItsBuilt: 'The decision is explicit rather than inferred from a score, keeping accountability with the reviewer.' },
    { id: 'dispatch', code: 'DP', name: 'Response dispatch', groupId: 'decision', size: 'm', position: { gx: 14, gy: 4 }, faceTexture: 'plain', properties: [{ id: 'p-dp-1', key: 'priority', value: 'confidence × urgency' }], whatItDoes: 'Routes a verified alert to the people who can act in the affected place.', howItsBuilt: 'Recipients are selected from geography and responsibility instead of a single broadcast list.' },
    { id: 'steward', code: 'ST', name: 'Local steward', groupId: 'outside', size: 's', position: { gx: 12, gy: 11 }, faceTexture: 'plain', properties: [{ id: 'p-st-1', key: 'response', value: 'inspect and report' }], whatItDoes: 'Checks the place, acts when needed and reports what changed after the intervention.', howItsBuilt: 'The response closes the evidence loop instead of ending at notification delivery.' },
    { id: 'habitat', code: 'HB', name: 'Habitat', groupId: 'outside', size: 'xl', position: { gx: 17, gy: 11 }, faceTexture: 'hatched', properties: [{ id: 'p-hb-1', key: 'state', value: 'observed over time' }], whatItDoes: 'The living place the whole system is trying to understand and protect.', howItsBuilt: 'It is modelled as a first-class concept so every alert and response remains tied to a real place.' },
  ],
  relations: [
