...
function StepNavigator({ flow, program, started, time, onOpenStage }: { flow: OntologyFlow; program: FlowProgram | null; started: boolean; time: number; onOpenStage: (stageId: string) => void }) {
  ...
  return <nav className="step-navigator" aria-label={`Steps in ${flow.name}`}>
    ...
    <ol ref={track} className="step-navigator-track">{flow.stages.map((stage, index) => {
      ...
      return <li key={stage.id}><button ref={current ? currentButton : undefined} type="button" disabled={!program} aria-current={current ? 'step' : undefined} aria-label={`${label}, ${index + 1} of ${flow.stages.length}`} title={label} onClick={() => { if (program) { seekFlowStage(program.id, stage.id); onOpenStage(stage.id) } }}>{String(index + 1).padStart(2, '0')}</button></li>
    })}</ol>
    <AppSelect className="step-navigator-select" compact ariaLabel={`Choose a step in ${flow.name}`} value={currentStageId} disabled={!program} options={flow.stages.map((stage, index) => ({ value: stage.id, label: stage.name || `Step ${String(index + 1).padStart(2, '0')}` }))} onChange={(stageId) => { if (program && stageId) { seekFlowStage(program.id, stageId); onOpenStage(stageId) } }} />
  </nav>
}
...
export function ScenarioControls({ flow, program, stepDisplayMode, onStepDisplayMode, onOpenStage }: { flow: OntologyFlow; program: FlowProgram | null; stepDisplayMode: StepDisplayMode; onStepDisplayMode: (mode: StepDisplayMode) => void; onOpenStage: (stageId: string) => void }) {
  ...
  return <section className="scenario-controls" style={{ '--scenario-player-width': `${500 + flow.stages.length * 28}px` } as CSSProperties} aria-label={`Scenario controls for ${flow.name}`}>
    ...
    <div className="step-display-control"><span>Labels</span><AppSelect compact ariaLabel="Step labels" value={stepDisplayMode} options={[{ value: 'all', label: 'All' }, { value: 'current', label: 'Current' }, { value: 'none', label: 'None' }]} onChange={(value) => onStepDisplayMode(value as StepDisplayMode)} /></div>
