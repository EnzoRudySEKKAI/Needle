...
function traversalCandidates(relations: readonly OntologyRelation[], frontier: ReadonlySet<string> | null): TraversalCandidate[] {
  return relations.flatMap((relation) => {
    const candidates: TraversalCandidate[] = []
    if (frontier === null || frontier.has(relation.from)) candidates.push({ relation, direction: 'forward' })
    if (frontier === null || frontier.has(relation.to)) candidates.push({ relation, direction: 'reverse' })
    return candidates
  })
}
...
function stagesAreContinuous(stages: readonly FlowStage[], relations: readonly OntologyRelation[]): boolean {
  const relationById = new Map(relations.map((relation) => [relation.id, relation]))
  let frontier: Set<string> | null = null
  for (const stage of stages) {
    if (stage.traversals.length === 0) return false
    const targets = new Set<string>()
    for (const traversal of stage.traversals) {
      const relation = relationById.get(traversal.relationId)
      if (!relation) return false
      const resolved = resolveTraversal(traversal, relation)
      if (frontier !== null && !frontier.has(resolved.sourceId)) return false
      targets.add(resolved.targetId)
    }
    frontier = targets
  }
...
export function ScenarioInspector({ flow, document, commit, editable }: { flow: OntologyFlow; document: OntologyDocument; commit: Commit; editable: boolean }) {
  ...
  const finalFrontier = flow.stages.length ? endpoints(flow.stages[flow.stages.length - 1]!, document.relations).targets : null
  const nextStageCandidates = traversalCandidates(document.relations, finalFrontier)
    ...
    <div className="form-stack">
      ...
      <div className="scenario-stages">
        {flow.stages.map((stage, stageIndex) => {
          const frontier = stageIndex === 0 ? null : endpoints(flow.stages[stageIndex - 1]!, document.relations).targets
          const usedRelationIds = new Set(stage.traversals.map((traversal) => traversal.relationId))
          const candidates = traversalCandidates(document.relations.filter((relation) => !usedRelationIds.has(relation.id)), frontier)
          ...
          return <section className="scenario-stage" key={stage.id}>
            <header><strong>Step {String(stageIndex + 1).padStart(2, '0')}</strong><div><button type="button" disabled={stageIndex === 0 || !stagesAreContinuous(moveUp, document.relations)} onClick={() => patch({ stages: moveUp })}>↑</button><button type="button" disabled={stageIndex === flow.stages.length - 1 || !stagesAreContinuous(moveDown, document.relations)} onClick={() => patch({ stages: moveDown })}>↓</button><button type="button" disabled={!stagesAreContinuous(withoutStage, document.relations)} onClick={() => patch({ stages: withoutStage })}>×</button></div></header>
            {stage.traversals.map((traversal) => {
              ...
              const resolved = resolveTraversal(traversal, relation)
              const reversed = flow.stages.map((item, index) => index === stageIndex ? { ...item, traversals: item.traversals.map((candidate) => candidate.id === traversal.id ? { ...candidate, direction: candidate.direction === 'forward' ? 'reverse' as const : 'forward' as const } : candidate) } : item)
              ...
              return <div className="scenario-branch" key={traversal.id}><span className="branch-code">{nodeById.get(resolved.sourceId)?.code}</span><span className="branch-name">{nodeById.get(resolved.sourceId)?.name}</span><button type="button" className="branch-direction" title="Reverse in this scenario" disabled={!stagesAreContinuous(reversed, document.relations)} onClick={() => patch({ stages: reversed })}>→</button><span className="branch-code">{nodeById.get(resolved.targetId)?.code}</span><span className="branch-name">{nodeById.get(resolved.targetId)?.name}</span><button type="button" className="branch-remove" disabled={!stagesAreContinuous(removed, document.relations)} onClick={() => patch({ stages: removed })}>×</button><small>{relation.label}</small></div>
            })}
