...
export function MapHeader({ activeFlowId, editable, stepDisplayMode, fullscreen, fullscreenError, onStepDisplayMode, onFullscreen, onEditable, onExport, leftCollapsed = false, rightCollapsed = false, headerCollapsed = false, onToggleLeft, onToggleRight, onToggleHeader }: { activeFlowId: string | null; editable: boolean; stepDisplayMode: StepDisplayMode; fullscreen: boolean; fullscreenError: string | null; onStepDisplayMode: (mode: StepDisplayMode) => void; onFullscreen: () => void; onEditable?: (editable: boolean) => void; onExport?: () => void; leftCollapsed?: boolean; rightCollapsed?: boolean; headerCollapsed?: boolean; onToggleLeft?: () => void; onToggleRight?: () => void; onToggleHeader?: () => void }) {
  ...
  return <header className="map-header">
    ...
    <div className="header-actions">
      ...
      {onToggleHeader ? <button type="button" className={`rail-toggle-button ${headerCollapsed ? 'is-collapsed' : ''}`} aria-label={headerCollapsed ? 'Show header' : 'Hide header'} aria-pressed={!headerCollapsed} title={headerCollapsed ? 'Show header (H)' : 'Hide header (H)'} onClick={onToggleHeader}><span aria-hidden="true">⌃</span></button> : null}
